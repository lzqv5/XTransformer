...
from src.transformer_settings.config import max_len, model_path
...
def registerHooks(model,data,params,iterOrder):
    ...
    model.encoder.layers[0].self_attn.register_forward_hook(
        utils.getSelfAttentionHook('encoder',data,params,iterOrder,blockOrder=1))

    model.encoder.layers[0].sublayer[0].norm.register_forward_hook(
        utils.getLayerNormHook('encoder',data,params,iterOrder,blockOrder=1,sublayerOrder=1))

    model.encoder.layers[0].feed_forward.register_forward_hook(
        utils.getFeedForwardHook('encoder',data,params,iterOrder,blockOrder=1))

    model.encoder.layers[0].sublayer[1].norm.register_forward_hook(
        utils.getLayerNormHook('encoder',data,params,iterOrder,blockOrder=1,sublayerOrder=2))

    model.encoder.layers[1].self_attn.register_forward_hook(
        utils.getSelfAttentionHook('encoder',data,params,iterOrder,blockOrder=2))

    model.encoder.layers[1].sublayer[0].norm.register_forward_hook(
        utils.getLayerNormHook('encoder',data,params,iterOrder,blockOrder=2,sublayerOrder=1))

    model.encoder.layers[1].feed_forward.register_forward_hook(
        utils.getFeedForwardHook('encoder',data,params,iterOrder,blockOrder=2))

    model.encoder.layers[1].sublayer[1].norm.register_forward_hook(
        utils.getLayerNormHook('encoder',data,params,iterOrder,blockOrder=2,sublayerOrder=2))
    ...
    model.decoder.layers[0].self_attn.register_forward_hook(
        utils.getSelfAttentionHook('decoder',data,params,iterOrder,blockOrder=1))

    model.decoder.layers[0].sublayer[0].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=1,sublayerOrder=1))

    model.decoder.layers[0].src_attn.register_forward_hook(
        utils.getCrossAttentionHook(data,params,iterOrder,blockOrder=1))

    model.decoder.layers[0].sublayer[1].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=1,sublayerOrder=2))

    model.decoder.layers[0].feed_forward.register_forward_hook(
        utils.getFeedForwardHook('decoder',data,params,iterOrder,blockOrder=1))

    model.decoder.layers[0].sublayer[2].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=1,sublayerOrder=3))

    model.decoder.layers[1].self_attn.register_forward_hook(
        utils.getSelfAttentionHook('decoder',data,params,iterOrder,blockOrder=2))

    model.decoder.layers[1].sublayer[0].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=2,sublayerOrder=1))

    model.decoder.layers[1].src_attn.register_forward_hook(
        utils.getCrossAttentionHook(data,params,iterOrder,blockOrder=2))

    model.decoder.layers[1].sublayer[1].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=2,sublayerOrder=2))

    model.decoder.layers[1].feed_forward.register_forward_hook(
        utils.getFeedForwardHook('decoder',data,params,iterOrder,blockOrder=2))

    model.decoder.layers[1].sublayer[2].norm.register_forward_hook(
        utils.getLayerNormHook('decoder',data,params,iterOrder,blockOrder=2,sublayerOrder=3))
